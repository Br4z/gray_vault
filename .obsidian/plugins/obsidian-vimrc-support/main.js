...
function jumpToPattern({
    ...
    const effectiveRepeat =
        repeat % orderedMatches.length || orderedMatches.length;
...
function jumpToHeading({ cm, cursorPosition, repeat, direction }) {
    ...
    const filterMatch = (match) =>
        !isMatchWithinCodeblock(match, codeblockMatches);
...
function isMatchWithinCodeblock(match, codeblockMatches) {
    return codeblockMatches.some((codeblockMatch) =>
        isWithinMatch(codeblockMatch, match.index),
    );
}
...
function defineAndMapObsidianVimAction(
    vimObject,
    vimrcPlugin,
    obsidianActionFn,
    mapping,
    ...
    vimObject.mapCommand(
        mapping,
        "action",
        obsidianActionFn.name,
        undefined,
        {},
    );
}
...
class VimrcPlugin extends obsidian.Plugin {
    constructor() {
        ...
        this.logVimModeChange = async (cm) => {
            ...
            if (this.settings.displayVimMode) this.updateVimStatusBar();
        };
        this.onVimKeypress = async (vimKey) => {
            if (vimKey != "<Esc>") {
                ...
                if (
                    this.customVimKeybinds[this.currentKeyChord.join("")] !=
                    undefined
                ) {
            ...
            if (this.settings.fixedNormalModeLayout) {
                ...
                if (
                    ...
                    if (view) {
                        ...
                        if (cmEditor) {
                            this.codeMirrorVimObject.handleKey(
                                cmEditor,
                                keyMap[ev.code],
                                "mapping",
                            );
    ...
    async updateVimEvents() {
        if (!this.app.isVimEnabled()) return;
        let view = this.getActiveView();
        if (view) {
            const cmEditor = this.getCodeMirror(view);
    ...
    async onload() {
        ...
        this.app.workspace.on("active-leaf-change", async () => {
            ...
            if (this.codeMirrorVimObject.loadedVimrc) return;
    ...
    defineBasicCommands(vimObject) {
        vimObject.defineOption(
            ...
            (value, cm) => {
                if (value) {
                    if (
                        value.trim() == "unnamed" ||
                        value.trim() == "unnamedplus"
                    ) {
                        if (!this.yankToSystemClipboard) {
                            this.yankToSystemClipboard = true;
                            console.log(
                                "Vim is now set to yank to system clipboard.",
                            );
                        }
                    } else {
                        throw new Error(
        ...
        vimObject.defineEx("iunmap", "", (cm, params) => {
            if (params.argString.trim()) {
                this.codeMirrorVimObject.unmap(
        ...
        vimObject.defineEx("noremap", "", (cm, params) => {
            ...
            if (params.argString.trim()) {
                this.codeMirrorVimObject.noremap.apply(
                ...
                if (key.startsWith("wait")) {
                    ...
                    try {
                        keyEvent = new KeyboardEvent(
                            "keydown",
                            keyboardeventFromElectronAccelerator.toKeyEvent(
                                key,
                            ),
                        );
                        ...
                        allGood = false;
                        throw new Error(
                            `Key '${key}' couldn't be read as an Electron Accelerator`,
                        );
                    }
                    if (allGood) {
                        for (keyEvent of events)
                            window.postMessage(
                                JSON.parse(JSON.stringify(keyEvent)),
                                "*",
                            );
    ...
    executeObsidianCommand(commandName) {
        ...
        if (!(commandName in availableCommands)) {
            throw new Error(
                `Command ${commandName} was not found, try 'obcommand' with no params to see in the developer console what's available`,
            );
        }
        const view = this.getActiveView();
        const editor = view.editor;
        const command = availableCommands[commandName];
        const { callback, checkCallback, editorCallback, editorCheckCallback } =
            command;
        if (editorCheckCallback) editorCheckCallback(false, editor, view);
            ...
            throw new Error(
                `Command ${commandName} doesn't have an Obsidian callback`,
            );
    }
    ...
    defineSurround(vimObject) {
        ...
        var surroundFunc = (params) => {
            ...
            let beginning = newArgs[0]
                .replace("\\\\", "\\")
                .replace("\\ ", " "); // Get the beginning surround text
            ...
            if (
                editor.posToOffset(chosenSelection.anchor) ===
                editor.posToOffset(chosenSelection.head)
            ) {
            ...
            if (
                ...
                [chosenSelection.anchor, chosenSelection.head] = [
                    chosenSelection.head,
                    chosenSelection.anchor,
                ];
            }
            let currText = editor.getRange(
                chosenSelection.anchor,
                chosenSelection.head,
            );
            editor.replaceRange(
        ...
        vimObject.defineEx("surround", "", (cm, params) => {
        ...
        vimObject.defineEx("pasteinto", "", (cm, params) => {
            ...
            surroundFunc([
                "[",
                "](" +
                    vimObject.getRegisterController().getRegister("yank")
                        .keyBuffer +
                    ")",
            ]);
        });
    ...
    async captureYankBuffer(win) {
        ...
        try {
            const currentClipboardText =
                await win.navigator.clipboard.readText();
    ...
    prepareVimModeDisplay() {
        if (this.settings.displayVimMode) {
            ...
            this.vimStatusBar.setText(
                this.settings.vimStatusPromptMap[
                    "normal" /* vimStatus.normal */
                ],
    ...
    defineJsFile(vimObject) {
        vimObject.defineEx("jsfile", "", async (cm, params) => {
            ...
            try {
                ...
                throw new Error(
                    `Cannot read file ${params.args[0]} from vault root: ${e.message}`,
                );
            }
    ...
    display() {
        ...
        new obsidian.Setting(containerEl)
            ...
            .addText((text) => {
                ...
                text.setValue(
                    this.plugin.settings.vimrcFileName ||
                        DEFAULT_SETTINGS.vimrcFileName,
                );
                text.onChange((value) => {
        ...
        new obsidian.Setting(containerEl)
            ...
            .addToggle((toggle) => {
                toggle.setValue(
                    this.plugin.settings.displayChord ||
                        DEFAULT_SETTINGS.displayChord,
                );
        ...
        new obsidian.Setting(containerEl)
            ...
            .addToggle((toggle) => {
                toggle.setValue(
                    this.plugin.settings.displayVimMode ||
        ...
        new obsidian.Setting(containerEl)
            ...
            .addButton(async (button) => {
                ...
                button.onClick(async () => {
                    this.plugin.settings.capturedKeyboardMap =
                        await this.plugin.captureKeyboardLayout();
                    this.plugin.saveSettings();
                });
            })
            .addToggle((toggle) => {
                toggle.setValue(
                    this.plugin.settings.fixedNormalModeLayout ||
        ...
        new obsidian.Setting(containerEl)
            ...
            .addToggle((toggle) => {
                toggle.setValue(
                    this.plugin.settings.supportJsCommands ??
        ...
        new obsidian.Setting(containerEl)
            ...
            .addText((text) => {
                ...
                text.onChange((value) => {
                    this.plugin.settings.vimStatusPromptMap.normal =
                        value || DEFAULT_SETTINGS.vimStatusPromptMap.normal;
                    this.plugin.saveSettings();
                });
            });
        new obsidian.Setting(containerEl)
            ...
            .addText((text) => {
                ...
                text.onChange((value) => {
                    this.plugin.settings.vimStatusPromptMap.insert =
                        value || DEFAULT_SETTINGS.vimStatusPromptMap.insert;
        ...
        new obsidian.Setting(containerEl)
            ...
            .addText((text) => {
                ...
                text.onChange((value) => {
                    this.plugin.settings.vimStatusPromptMap.visual =
                        value || DEFAULT_SETTINGS.vimStatusPromptMap.visual;
                    this.plugin.saveSettings();
                });
            });
        new obsidian.Setting(containerEl)
            ...
            .addText((text) => {
                ...
                text.onChange((value) => {
                    this.plugin.settings.vimStatusPromptMap.replace =
                        value || DEFAULT_SETTINGS.vimStatusPromptMap.replace;
                    this.plugin.saveSettings();
                });
            });
    }
