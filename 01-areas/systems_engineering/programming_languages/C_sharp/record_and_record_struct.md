...
It declares a [[personal/systems_engineering/programming_languajes/C_sharp/value_types_vs_reference_types#Reference types|reference type]] (like `class`), but with value-like equality by default. Two records are considered equal when their contents (their components) are equal, not when they are the same instance.
...
It declares a [[personal/systems_engineering/programming_languajes/C_sharp/value_types_vs_reference_types#Value types|value type]] (like `struct`) with record-style features. it is copied by value by default, and it still provides value-like equality based on contents.
